...
if __name__ == "__main__":
    ...
    data = torch.load(
        f"./experiments/{experiment}/ckp/model_step_{model_step}.pt", map_location=device
    )
    ...
    for index in range(41):
        phi = phis[index]
        data_init = np.load(f"chevron/init/init_{index}.npz")["predictions"]
        particles = torch.tensor(np.ones(n + n_boundary, dtype=int), dtype=int, device=device)
        particles[:n_boundary] = 0
    
        for rep in range(reps):
            init = torch.tensor(data_init[rep], dtype=dtype, device=device)

            predictions = torch.zeros(size=(total_records, 3, init.shape[1]), dtype=dtype, device=device)
            ...
            np.savez(
                f"chevron/data/ml_{index}_{rep}",

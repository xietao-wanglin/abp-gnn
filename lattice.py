...
dtype = torch.float
model_step = 20000
sim_loc = f"./datasets/{cfg.dataset}/data/simulation_long_test_1.npy"
particle_loc = f"./datasets/{cfg.dataset}/data/particle_long_test_1.npy"
timesteps = 1600
...
for lc in lcs:
    ...
    print(f"Density: {density:.2f} - Spacing: {lc}")

    sims = run_sim(particles, initial_state, box_length, n_sims=200)
    np.save(f"./analysis/data/density-{density:.2f}.npy", sims)

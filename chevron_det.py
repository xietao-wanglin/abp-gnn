...
def generate_state_chevron(n, box_length, delta, n_boundary=30, chevron_angle=60, arm_length=0.4):
    ...
    for index, phi in enumerate(phis):
        ...
        for rep in tqdm(range(reps)):
            ...
            initial_state, particle_type = generate_state_chevron(
                n=n, chevron_angle=phi, n_boundary=20, box_length=box_length, arm_length=0.3, delta=0.1
            )
            sim = TorqueWCA(
            ...
            out = sim.solve_dynamics(
                t_end=10, dt=1e-6, save_dt=save_dt, debug=True, use_controller=True
            )
            res = np.array(out.block_until_ready())[-1]
            all_reps_results.append(res)
